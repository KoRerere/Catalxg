...
const pages=['product/retatrutide-20mg-rd-only','product/bpc-157-and-tb-500','product/glow-70mg','product/nad-1000mg','product/tirzepatide','product/where-to-buy-retatrutide'];
  ...
  try{
    ...
    const data=await page.evaluate(()=>{
      ...
      for(const rw of rows){
        ...
        const lp=Math.round(minLeft/692*100), rp=Math.round(maxRight/692*100);
        hits.push({bg, leftPct:lp, rightPct:rp, hasIcon:!!rw.querySelector('i.fontawesome-icon'),
          text:(rw.querySelector('h1,h2,h3,h4,strong,li,p')?.textContent||'').trim().slice(0,40)});
      }
      return hits;
    });
